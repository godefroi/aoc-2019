...
namespace aoc_2019
{
	...
	{
		private static string m_input = @"FGY)61Z
2BN)LM7
QXY)TVB
5M5)Y6C
2L2)64M
...
1LV)GFH";
		...
		// started ~7:15, ended ~7:41
		public static void Part1()
		{
			//using( var sr = new StringReader("COM)B\nB)C\nC)D\nD)E\nE)F\nB)G\nG)H\nD)I\nE)J\nJ)K\nK)L") ) {
			(var ndic, var com) = BuildTree(m_input);
		...
		// started 7:45, finished 8:06
		public static void Part2()
		{
			(var ndic, _) = BuildTree(m_input);
			//(var ndic, _) = BuildTree("COM)B\nB)C\nC)D\nD)E\nE)F\nB)G\nG)H\nD)I\nE)J\nJ)K\nK)L\nK)YOU\nI)SAN");
		...
		private static (Dictionary<string, Node> tree, Node com) BuildTree(string input)
		{
			...
			using( var sr = new StringReader(input) ) {
				while( sr.Peek() > -1 ) {
					(var parent, var child) = sr.ReadLine().Split(")");
